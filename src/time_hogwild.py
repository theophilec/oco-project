...
if __name__ == '__main__':
            ...
            for seed in range(n_runs):
                if algo == 'hogwild':
                    dt, T, test_error = train_hogwild(
                        a=x_train, b=y_train, a_test=x_test, b_test=y_test, T=T_config, alpha=alpha, beta=beta, K=K,
                        theta=theta, n_processes=n_workers, sequential=False, seed=seed, use_logger=False
                    )
                elif algo == 'hogwild_seq':
                    dt, T, test_error = train_hogwild(
                        a=x_train, b=y_train, a_test=x_test, b_test=y_test, T=T_config, alpha=alpha, beta=beta, K=K,
                        theta=theta, n_processes=1, sequential=True, seed=seed, use_logger=False
                    )
                elif algo == 'sgd':
                    dt, T, test_error = train_sgd(
                        a=x_train, b=y_train, a_test=x_test, b_test=y_test, T=T_config, alpha=alpha, return_avg=True,
                        seed=seed, use_logger=False
                    )
    ...
    df.to_csv(save_folder.joinpath(f'{datetime.now().strftime("%H%M%S")}.csv'), index=False)
