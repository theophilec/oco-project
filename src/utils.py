...
class AvgLogger:
    def __init__(self, loggers: List[Logger]):
        ...
        assert max([len(log.iterations) for log in loggers]) == min(
            [len(log.iterations) for log in loggers]
        )
        ...
        self.time_elapsed = np.array([log.time_elapsed for log in loggers]).T.mean(
            axis=1
        )

        self.test_error_std = np.array([log.test_error for log in loggers]).T.std(
            axis=1
        )
        self.test_error_p95 = np.percentile(
            np.array([log.test_error for log in loggers]).T, 95, axis=1
        )
        self.test_error_p5 = np.percentile(
            np.array([log.test_error for log in loggers]).T, 5, axis=1
        )
        self.time_elapsed_p95 = np.percentile(
            np.array([log.time_elapsed for log in loggers]).T, 95, axis=1
        )
        self.time_elapsed_p5 = np.percentile(
            np.array([log.time_elapsed for log in loggers]).T, 5, axis=1
        )
...
def plot_results(loggers: List[Union[Logger, AvgLogger]], add_to_title=""):
    ...
    for logger in loggers:
        ...
        if len(logger.time_elapsed) > 0:
            ...
            if plot_std:
                ax3.fill_between(
                    x, logger.time_elapsed_p5, logger.time_elapsed_p95, alpha=0.33
                )
